...
```{r}
table(Data1$trt_anx,Data1$trt_the)
...
```{r}
t <- table(Data1$trt_anx, Data1$cgis_adm)
chisq.test(t)
...
```{r}
t <- table(Data1$trt_anx, Data1$medical_history_ren)
chisq.test(t)
...
```{r}
ggplot(Data1, aes(trt_anx, after_stat(count))) + geom_bar(aes(fill = medical_history_ren), position = "dodge")
...
Data2 <- Data1[,c("trt_anx","medical_history_dia", "medical_history_sud","medical_history_hbp",
...
i <- lm(trt_anx ~ 1, data=no.na.data2)

#define model with all predictors
all <- lm(trt_anx ~ ., data=no.na.data2)
...
```
From this backwards selection in finding what variables are predictive of antidepressants being given as treatment, substance abuse was positively correlated, diagnosis of renal failure was negatively correlated and currently feeling depressed or hopeless were all reasons in why the patient received antidepressants.
